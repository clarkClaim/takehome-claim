...
export class PayoutProcessor {
    ...
    private static handleApproved = async (payout: payout): Promise<payout> => {
        ...
        try {
            ...
            return await PayoutProcessor.processUpdatedPayout(processingPayout, payout);
        } catch (error) {
            console.log(`Error processing approved payout ${payout.id}:`, error);
    ...
    private static handleProcessing = async (payout: payout): Promise<payout> => {
        ...
        try {
            ...
            return await PayoutProcessor.processUpdatedPayout(settledPayout, payout);
        } catch (error) {
        ...
        try {
            switch (updatedPayout.status) {
                ...
                case payout_status.PROCESSING:
                    return await PayoutProcessor.handleProcessing(updatedPayout);
                
                case payout_status.SETTLED:
                    console.log(`Payout ${updatedPayout.id} settled successfully`);
                    return updatedPayout;
                ...
                default:
                    console.log(`Invalid payout status for id ${updatedPayout.id}: ${updatedPayout.status}`);
                    return await PayoutProcessor.updatePayoutStatus(
                        updatedPayout.id, 
                        payout_status.FAILED
                    );
            ...
            console.log(`Unexpected error processing payout ${updatedPayout.id}:`, error);
            return await PayoutProcessor.updatePayoutStatus(
                updatedPayout.id, 
                payout_status.FAILED
            );
